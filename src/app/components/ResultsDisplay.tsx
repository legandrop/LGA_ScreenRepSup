...
export default function ResultsDisplay(props: ResultsDisplayProps) {
  const { cameraMovement, soporte, chroma, semitransparente, reflejoImportante, iluminacion, tvOnOff, basePath, lga_debug } = props;

  console.log('ResultsDisplay received props:', props);

  const renderResult = () => {
    if (iluminacion === 'alta' && !chroma && !cameraMovement) {
    ...
    if ((iluminacion === 'media' || iluminacion === 'baja') && !chroma && !cameraMovement) {
